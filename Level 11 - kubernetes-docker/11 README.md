...
2. **Enable CloudWatch Container Insights:**  
   **a. Create IAM Policy**  
  Save this JSON as `CloudWatchAgentServerPolicy.json`:
   ```json
   {
     "Version": "2012-10-17",
     "Statement": [
   ...
   aws iam create-policy \
     ...
     --policy-document file://CloudWatchAgentServerPolicy.json
   ```

   **b. Create ServiceAccount with IRSA**  
     > **NOTE** Use eksctl to make a ServiceAccount in the amazon-cloudwatch namespace and attach your new policy. This gives pods using that ServiceAccount the right AWS permissions—no node‑wide IAM tricks needed:
   ```bash
   eksctl create iamserviceaccount \
     ...
     --attach-policy-arn arn:aws:iam::<YOUR_ACCOUNT_ID>:policy/CloudWatchAgentServerPolicy \
     --override-existing-serviceaccounts \
     --approve
   ```

   **c. Deploy CloudWatch Agent**  
   > **NOTE** AWS publishes sample manifests on GitHub. Apply them in this order:
   ```bash
   kubectl apply -f https://raw.githubusercontent.com/aws-samples/amazon-cloudwatch-container-insights/latest/k8s-deployment-manifest-templates/deployment-mode/DaemonSet/container-insights/eks/cwagent-configmap.yaml
   kubectl apply -f https://raw.githubusercontent.com/aws-samples/amazon-cloudwatch-container-insights/latest/k8s-deployment-manifest-templates/deployment-mode/DaemonSet/container-insights/eks/cwagent-daemonset.yaml
   ```
...
1. **View Application Logs:**  
   ...
   ```
   Replace `<pod-name>` with the actual pod name.  
   > **Why this helps:** Logs show live errors and output so you can quickly pinpoint and fix issues.
...
eksctl delete cluster --name demo-cluster --region us-east-1
```  
This removes the VPC, worker nodes, LoadBalancer, and all associated AWS resources.
